...
export const Table = ({
  ...
  return (
    <div>
      <div className="w-full mb-8 border border-accent-primary overflow-x-auto block rounded-lg min-h-[30.3rem] bg-white dark:bg-background">
        ...
        <table className="w-full h-full rounded-lg">
            ...
            {table?.getHeaderGroups()?.map((headerGroup, i) => (
              <tr key={i}>
                {headerGroup.headers.map((header, idx) => (
                  <th
                    key={idx}
                    className="bg-[#F9FAFB] dark:bg-background font-normal text-sm text-[#6C748B] whitespace-nowrap py-5 px-5 text-left capitalize"
                  >
          ...
          <tbody>
            ...
            {table.getRowModel().rows.map((row, index) => (
              <tr
                key={index}
                className="border-t-[1.3px] bg-[#FFFFFF] dark:bg-background border-accent-primary dark:border-t-[1.3px] text-[14px] text-sm text-[#6C748B] font-normal cursor-pointer whitespace-nowrap hover:bg-[#F9FAFB]"
              >
